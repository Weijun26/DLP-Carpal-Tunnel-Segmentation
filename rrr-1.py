...
EPOCHS_PER_ROUND = 10
START_FROM_EPOCH = 0 # [設定] 通常是從 0 開始
BATCH_SIZE = 12 
LR = 1e-4  # 固定 LR
...
def train_one_fold(fold_index, train_indices, val_indices, current_target_epoch):
    ...
    for epoch in range(start_epoch, current_target_epoch):
        ...
        print(f"   Ep {epoch+1} Loss: {avg_val:.4f} | MN: {np.mean(mn):.2f} | FT: {np.mean(ft):.2f} | CT: {np.mean(ct):.2f}")
...
def main_gui():
    ...
    tk.Label(root, text="DLP 訓練控制器\n(Standard AdamW)", font=("Arial", 12)).pack(pady=20)
